...
namespace Clustering{
  ...
  void PointsSpace::init_points_from_file(std::string filename)
  { 
    std::ifstream fp;
    ...
    {
      std::vector<std::string> coordinateStrings;
      boost::split(coordinateStrings, line, boost::is_any_of("\t"));
      Point p;
      BOOST_FOREACH(std::string& coordinateString, coordinateStrings)
      {
        Coord coord;
        std::stringstream(coordinateString) >> coord;
        p.push_back(coord);
      }
      points__.push_back(p);
      num_dimensions__ = p.size();
      std::cout << "pid[" << i << "]= (" << p << ")" <<std::endl;; 
      ++i;
    }
    fp.close();
    num_points__ = points__.size();
  }
